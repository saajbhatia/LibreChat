...
describe('LearnLight tool-service response limits', () => {
  const originalFetch = global.fetch;
  const originalServiceKey = process.env.LEARNLIGHT_SERVICE_KEY;

  beforeEach(() => {
    process.env.LEARNLIGHT_SERVICE_KEY = 'learnlight-response-limit-test-key';
  });
  ...
  it('retains a bounded service error body after streaming it', async () => {
    ...
    await expect(getAssignments({ tenantId: 'tenant-1' })).rejects.toThrow(
      'LearnLight service responded 503 for /api/learnlight/assignments: ' +
