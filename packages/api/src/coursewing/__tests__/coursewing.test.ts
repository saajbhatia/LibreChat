...
import type { CourseWingCourseContext } from '../types';

const courseContext: CourseWingCourseContext = {
  course: {
    canvasCourseId: 754,
    name: 'AP Chemistry',
    courseCode: 'CHEM-AP',
...
describe('buildAssistancePolicy', () => {
  it('builds a marker-prefixed policy block for every level', () => {
    for (const level of assistanceLevels) {
      ...
      expect(policy.startsWith(COURSEWING_POLICY_MARKER)).toBe(true);
...
describe('buildLearningDefault', () => {
  it('builds a tutor block without policy framing', () => {
    ...
    expect(tutor.startsWith(COURSEWING_TUTOR_MARKER)).toBe(true);
    expect(tutor).toContain('Answer direct questions fully');
    expect(tutor).not.toContain(COURSEWING_POLICY_MARKER);
  ...
  it('tells the tutor to ground school-life questions in real data via tools', () => {
    const tutor = buildLearningDefault();
    expect(tutor).toContain('coursewing_* tools');
...
describe('stripCourseWingBlocks', () => {
  it('strips a server-appended learning default block', () => {
    ...
    expect(stripCourseWingBlocks(prefix)).toBe('Some base prefix');
  });
});
...
describe('buildCourseCard', () => {
  it('renders a compact card with course, assignments, and announcements', () => {
    ...
    expect(card).toContain('coursewing_search_materials');
  ...
  it('quotes Canvas prompt-injection text inside a non-closeable untrusted-data block', () => {
    ...
    expect(card.indexOf('Tools: coursewing_get_assignments')).toBeGreaterThan(
  ...
  it('caps large course cards by UTF-8 bytes while retaining the trusted tool instructions', () => {
    ...
    expect(card).toContain('Tools: coursewing_get_assignments');
  });
...
describe('coursewing tools', () => {
  const originalFetch = global.fetch;
  const originalServiceKey = process.env.COURSEWING_SERVICE_KEY;

  beforeEach(() => {
    process.env.COURSEWING_SERVICE_KEY = 'coursewing-test-service-key';
  });
  ...
  it('returns assignment JSON from the service', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining('/api/coursewing/courses/754/assignments?filter=upcoming'),
      expect.anything(),
    );
  });
  ...
  it('suggests alternatives when search has no hits', async () => {
    mockFetchByUrl([
      { match: '/api/coursewing/search', payload: { query: 'entropy', hits: [] } },
      {
    ...
    const result = await createCourseWingTool(COURSEWING_SEARCH_MATERIALS, {
      tenantId: 'tenant-1',
  ...
  it('sends app feedback without collecting conversation metadata', async () => {
    ...
    const result = await createCourseWingTool(COURSEWING_SEND_FEEDBACK, {
      tenantId: 'tenant-1',
      userName: 'Saaj',
    ...
    expect(String(url)).toContain('/api/coursewing/feedback');
  ...
  it('does not send feedback when the message is missing', async () => {
    ...
    const result = await createCourseWingTool(COURSEWING_SEND_FEEDBACK).invoke({});
