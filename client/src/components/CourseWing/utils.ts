...
import type { CourseWingAssignment, CourseWingCourseSummary } from '~/data-provider/CourseWing';
...
export type CourseWingCourseIdentity = Pick<
  CourseWingCourseSummary,
...
/** Demo/testing override: `VITE_COURSEWING_FAKE_NOW` makes course views pretend "now" is that instant. */
export function coursewingNow(): Date {
...
export function getCoursePrefix(course: CourseWingCourseIdentity): string {
  return [
...
export function getAssignmentPrefix(
  course: CourseWingCourseIdentity,
...
export function getReviewPrefix(course: CourseWingCourseIdentity): string {
  return [
    getCoursePrefix(course),
    ...
    'Use tools when they are needed, not as a ritual: the card covers setup, but when the student asks what the exam covers, about specific course content, or how something was taught in class, look it up (coursewing_get_modules for structure, coursewing_search_materials/coursewing_read_material for content) with one or two targeted calls rather than answering from general knowledge. Never bulk-read materials up front.',
  ].join('\n');
}
...
export const COURSEWING_TEACHER_ASSISTANT_LINE = 'CourseWing teacher assistant';
...
export function openTeacherAssistantChat(
  ...
  course: CourseWingCourseIdentity & { name: string },
  options: { spec?: string; prompt?: string } = {},
): boolean {
  const promptPrefix = [
    `Canvas course ID: ${course.canvasCourseId}`,
    `Current Canvas course: ${prefixValue(course.name)}`,
    `${COURSEWING_TEACHER_ASSISTANT_LINE}: class`,
...
export function openCourseChat(
  ...
  navigate(`/c/${Constants.NEW_CONVO}?coursewing=${encodeURIComponent(handoffId)}`, {
    state: { focusChat: true },
  });
  return true;
}

const PENDING_GREETING_KEY = 'coursewing:pendingGreeting';
const COURSE_CHAT_HANDOFF_PREFIX = 'coursewing:chat-handoff:';
const PENDING_COURSE_EVENT = 'coursewing:pending-course-changed';
...
/** Reads and deletes a single CourseWing navigation handoff. */
