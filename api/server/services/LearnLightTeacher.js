...
const { serviceFetch, getLearnLightCanvasIdentity } = require('~/server/services/LearnLight');

const TEACHER_MODEL = () =>
  process.env.LEARNLIGHT_TEACHER_MODEL || 'us.anthropic.claude-haiku-4-5-20251001-v1:0';
...
const LearnLightReceipt =
  mongoose.models.LearnLightReceipt || mongoose.model('LearnLightReceipt', receiptSchema);
const LearnLightCourseSetting =
  mongoose.models.LearnLightCourseSetting ||
  mongoose.model('LearnLightCourseSetting', courseSettingSchema);
const LearnLightActivity =
  mongoose.models.LearnLightActivity || mongoose.model('LearnLightActivity', activitySchema);

const teacherRoleSchema = new mongoose.Schema(
  { email: { type: String, required: true, unique: true, lowercase: true } },
  { timestamps: true },
);
const LearnLightTeacherRole =
  mongoose.models.LearnLightTeacherRole ||
  mongoose.model('LearnLightTeacherRole', teacherRoleSchema);
...
async function getAssignedTeacherEmails() {
  ...
  const docs = await LearnLightTeacherRole.find().select('email').lean();
...
function envTeacherEmails() {
  return (process.env.LEARNLIGHT_TEACHER_EMAILS ?? '')
    .split(',')
    .map((value) => value.trim().toLowerCase())
    .filter(Boolean);
}

/** Teacher role: ADMIN, an env-listed email, or an email assigned in the LearnLightTeacherRole collection. */
...
async function getTeacherCourseIds(userId) {
  ...
  const { ok, body } = await serviceFetch('/api/learnlight/courses/current', {
...
async function getCourseSetting(canvasCourseId) {
  ...
  const setting = await LearnLightCourseSetting.findOne({ canvasCourseId }).lean();
...
async function getCourseAssistance(canvasCourseId, canvasAssignmentId) {
  try {
    ...
    logger.error('[LearnLightTeacher] Failed to resolve course assistance setting', error);
    return { policyLevel: null, blockedRules: [] };
  }
}
...
async function llmChat(messages) {
  ...
  try {
    ...
    logger.error(`[LearnLightTeacher] LLM call failed: ${error.message}`);
    return null;
  }
}
...
async function llmJson(messages) {
  ...
  try {
    ...
    logger.error('[LearnLightTeacher] LLM returned unparseable JSON');
    return null;
  }
}
...
async function generateReceipt(conversation, stats) {
  ...
  const existing = await LearnLightReceipt.findOne({
  ...
  return LearnLightReceipt.findOneAndUpdate(
    { conversationId: conversation.conversationId },
    { $set: update },
...
async function refreshReceipts(canvasCourseId, { limit = MAX_RECEIPTS_PER_REFRESH } = {}) {
  ...
  const [countRows, receipts] = await Promise.all([
    ...
    LearnLightReceipt.find({ conversationId: { $in: ids } })
  ...
  const workers = Array.from({ length: Math.min(RECEIPT_CONCURRENCY, queue.length) }, async () => {
    while (queue.length) {
      ...
      try {
        ...
        logger.error(
          `[LearnLightTeacher] Receipt generation failed for ${conversation.conversationId}`,
          error,
        );
      }
    }
...
const TEACHER_ASSISTANT_LINE = 'LearnLight teacher assistant';

function isTeacherAssistantPrefix(promptPrefix) {
  return (
    typeof promptPrefix === 'string' && /^\s*LearnLight teacher assistant\b/im.test(promptPrefix)
  );
}

function buildTeacherAssistantPrompt() {
  return `[LearnLight tutor — class assistant]
...
You may use the LearnLight course tools to look up course material, assignments, and dates. Help the teacher analyze patterns, check on specific students, and draft activities, warm-ups, or announcements. Be concise and concrete.`;
}
...
async function buildClassReceiptsCard(canvasCourseId) {
  const receipts = await LearnLightReceipt.find({ canvasCourseId })
    .sort({ lastMessageAt: -1 })
    .limit(60)
    .lean();
  if (!receipts.length) {
    return '[LearnLight class receipts]\nNo tutor sessions have been recorded for this course yet.';
  }
