...
async function learnLightContext(req, res, next) {
  ...
  try {
    ...
    if (canvasCourseId != null) {
      const identity = await getLearnLightCanvasIdentity(req.user?.id);
      if (!identity) {
        return res
          .status(409)
      ...
      // request. In particular, LearnLight tool construction must not resolve
    ...
    logger.error('[learnLightContext] Failed to verify Canvas course scope', error);
