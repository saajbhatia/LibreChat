...
jest.mock('librechat-data-provider', () => ({
  ...
  stripLearnLightBlocks: jest.fn((prefix) => prefix),
...
describe('learnLightContext persisted Canvas scope', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  test('uses the persisted course id and hidden matching account key for an existing chat', async () => {
    ...
    await learnLightContext(req, res, next);
    ...
    expect(req.learnLightCanvasTenantId).toBe('tenant-1');
    expect(req.learnLightCanvasAccountKey).toBe(CURRENT_ACCOUNT_KEY);
  ...
  test('rejects an omitted canonical marker on a persisted course chat', async () => {
    ...
    expect(getLearnLightCanvasIdentity).not.toHaveBeenCalled();
  });
  ...
  test('treats duplicate course markers as non-canonical and rejects the persisted course request', async () => {
    ...
    await learnLightContext(req, res, jest.fn());
