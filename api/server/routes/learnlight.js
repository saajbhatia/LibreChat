...
const { isValidLearnLightTenantId } = require('~/server/services/LearnLightAuth');
...
router.use((req, res, next) => {
  ...
  return res.status(404).json({ message: 'LearnLight is disabled' });
});

function tenantHeaders(tenantId, extra = {}) {
  if (tenantId != null && !isValidLearnLightTenantId(tenantId)) {
...
async function cleanupUnlinkedTenant(tenantId) {
  try {
    const cleanup = await serviceFetch(`/api/learnlight/tenants/${tenantId}`, {
      method: 'DELETE',
    });
    if (tenantDeleteFailed(cleanup)) {
      logger.error(`[learnlight/canvas] Failed to clean up unlinked Canvas tenant ${tenantId}`);
    }
  } catch (cleanupError) {
    logger.error('[learnlight/canvas] Failed to clean up an unlinked Canvas tenant', cleanupError);
  }
}
...
function parsePendingRevocation(value) {
  ...
  try {
    ...
    if (
      ...
      isValidLearnLightTenantId(parsed.tenantId) &&
      typeof parsed?.replacementTenantId === 'string' &&
      isValidLearnLightTenantId(parsed.replacementTenantId)
    ) {
      return parsed;
    }
  } catch {
...
async function retryPendingRevocation(userId) {
  ...
  const currentTenantId = await getLearnLightTenantId(userId);
  if (!pending || currentTenantId === pending.tenantId) {
    await clearPendingRevocation(userId);
    return true;
  }
  const deletion = await serviceFetch(`/api/learnlight/tenants/${pending.tenantId}`, {
    method: 'DELETE',
  });
  if (tenantDeleteFailed(deletion)) {
    return false;
  }
  clearLearnLightCanvasIdentityCache(pending.tenantId);
  await clearPendingRevocation(userId);
  return true;
}
...
router.put('/canvas', requireJwtAuth, async (req, res) => {
  ...
  try {
    ...
    const previousTenantId = await getLearnLightTenantId(req.user.id);
    ...
    if (!isValidLearnLightTenantId(newTenantId)) {
    ...
    if (updateResult instanceof Error) {
      if (isReplacement) {
        try {
          ...
          logger.error(
            '[learnlight/canvas] Failed to clear replacement cleanup marker',
            pendingError,
          );
        }
      }
    ...
    if (previousTenantId && previousTenantId !== newTenantId) {
      const deletion = await serviceFetch(`/api/learnlight/tenants/${previousTenantId}`, {
        method: 'DELETE',
      });
      if (tenantDeleteFailed(deletion)) {
        logger.error(
          `[learnlight/canvas] Will retry revoking replaced Canvas tenant ${previousTenantId}`,
        );
        return res.json({
          connected: true,
          isDefault: false,
      ...
      clearLearnLightCanvasIdentityCache(previousTenantId);
      await clearPendingRevocation(req.user.id);
    }

    clearLearnLightCanvasIdentityCache(newTenantId);
...
router.delete('/canvas', requireJwtAuth, async (req, res) => {
  ...
  try {
    ...
    for (const id of tenantIds) {
      const deletion = await serviceFetch(`/api/learnlight/tenants/${id}`, {
        method: 'DELETE',
      });
      if (tenantDeleteFailed(deletion)) {
        return res.status(502).json({ message: 'Canvas service could not revoke the connection' });
      }
      clearLearnLightCanvasIdentityCache(id);
    }
    await deleteLocalCanvasAuth(req.user.id);

    const fallback = await serviceFetch('/api/learnlight/tenant');
    if (!fallback.ok) {
...
router.get('/courses/current', optionalJwtAuth, async (req, res) => {
  try {
    ...
    logger.error('[learnlight/courses/current] Failed to fetch courses', error);
...
router.get('/courses/:canvasCourseId', optionalJwtAuth, async (req, res) => {
  ...
  try {
    return await proxyUserCanvasData(req, res, `/api/learnlight/courses/${canvasCourseId}`);
  } catch (error) {
    logger.error('[learnlight/courses/:canvasCourseId] Failed to fetch course materials', error);
    return res.status(502).json({ message: 'Canvas data unavailable' });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const created = await serviceFetch('/api/learnlight/feedback', {
        method: 'POST',
      ...
      if (shareChat) {
        ...
        const shared = await serviceFetch('/api/learnlight/feedback', {
          method: 'POST',
          headers: tenantHeaders(tenantId, {
      ...
      logger.error('[learnlight/feedback] Failed to send feedback', error);
      return res.status(502).json({ message: 'Sending feedback failed' });
    }
  },
);
